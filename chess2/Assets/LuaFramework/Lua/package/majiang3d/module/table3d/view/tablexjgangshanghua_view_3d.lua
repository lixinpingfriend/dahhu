...
---@class TableXJGangShangHuaView_3d:Table3dCommonView
local TableXJGangShangHuaView_3d = class('tableXJGangShangHuaView_3d', Base)
...
function TableXJGangShangHuaView_3d:init_ui()
    Base.init_ui(self)
...
function TableXJGangShangHuaView_3d:reset_state()
    Base.reset_state(self)
    self.xjGangShangHuaTip:SetActive(false)
end

--- 初始化单个座位
function TableXJGangShangHuaView_3d:init_seat(seatHolder, index)
    seatHolder.objYaHu = GetComponentWithPath(seatHolder.seatRoot, "Info/ImageYa", ComponentTypeName.Image).gameObject
end

---癞根
function TableXJGangShangHuaView_3d:game_state_lai_gen(show)
...
---开始设置玩家状态（gameState）
function TableXJGangShangHuaView_3d:game_state_begin_set_player_state(playerState, localSeat, serverSeat)
...
--- 开始刷新gameState
function TableXJGangShangHuaView_3d:game_state_begin(gameState)
    Base.game_state_begin(self, gameState)
    self:show_ke_ya_hua(#gameState.KeYaHu > 0)
end

---可压胡
function TableXJGangShangHuaView_3d:show_ke_ya_hua(show)
...
return  TableXJGangShangHuaView_3d