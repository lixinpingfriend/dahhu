...
---@class TableChuXiongErWuTong:TableCommonView
local tableChuXiongErWuTong = class('TableChuXiongErWuTong', Base)
...
function tableChuXiongErWuTong:init_config()
    Base.init_config(self)
    self.actionSounds["4"] = {"dianpao1"}
end

function tableChuXiongErWuTong:game_state_lai_gen(show)
    ...
    ---@type Mj2D
    local mj = Mj2D:new(22, nil, {gameObject = self.laiGenMJ, spriteHolder = self.frontSpriteH})
...
function tableChuXiongErWuTong:game_state_wei_zhang(show)
    show = true --self.allReady or (nil ~= self.gameState)
    ...
    ---@type Mj2D
    local mj = Mj2D:new(23, nil, {gameObject = self.WeiZhang, spriteHolder = self.frontSpriteH})
    mj:set_skin(self.mjColorSet, self.mjScaleSet)
    MjManager.insert(mj, MjManager.mjType.custom)
...
function tableChuXiongErWuTong:game_state_end_hand(serverSeat)
    ...
    if self:is_me(localSeat) then
        ...
        for i=1,#keGang do
            ...
            for j = #mjs,1,-1 do
                ...
                ---@type Mj2D
                local mj2d = mjs[j]
                if gangPai == mj2d.pai and true then
                    mj2d:set_tag(10, mj2d.skinObj)
                    mj2d.isKeGang = true
...
function tableChuXiongErWuTong:play_action_tx(action, seatInfo)
    ---@type SeatHolder2D
...
function tableChuXiongErWuTong:game_state_dun()
