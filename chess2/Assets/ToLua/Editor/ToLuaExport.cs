...
public static class ToLuaExport
{
    ...
    class _MethodBase
    {
        ...
        public int ProcessParams(int tab, bool beConstruct, int checkTypePos)
        {
            ...
            if ((!method.IsStatic && !beConstruct) || BeExtend)
            {
                ...
                else
                {
                    if (method.Name == "Equals")
                    {
                        ...
                        {
                            sb.AppendFormat("{0}{1} {2}obj{3} = ({1})ToLua.ToObject(L{3}, 1);\r\n", head, className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                        }
                    }
            ...
            for (int j = 0; j < count; j++)
            {
                ...
                string arg = string.Format("{1}arg{0}{2}", j, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            ...
            for (int j = 0; j < count; j++)
            {
                ...
                {
                    sbArgs.Append(string.Format("{0}arg",CustomNameSetting.variablePrefixName));
                }
                else
                {
                    if (param.Attributes == ParameterAttributes.Out)
                    {
                        sbArgs.Append(string.Format("out {0}arg", CustomNameSetting.variablePrefixName));
                    }
                    else
                    {
                        sbArgs.Append(string.Format("ref {0}arg", CustomNameSetting.variablePrefixName));
                    }

                    refList.Add(string.Format("{1}arg{0}{2}" , j,CustomNameSetting.variablePrefixName,CustomNameSetting.variableSuffixName) );
                    refTypes.Add(GetRefBaseType(param.ParameterType));
                }

                sbArgs.Append(string.Format("{0}{1}", j,CustomNameSetting.variableSuffixName));
            ...
            if (beConstruct)
            {
                sb.AppendFormat("{2}{0} {3}obj{4} = new {0}({1});\r\n", className, sbArgs.ToString(), head, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                string str = GetPushFunction(type);
                sb.AppendFormat("{0}ToLua.{1}(L{3}, {2}obj{3});\r\n", head, str, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            ...
            string obj = (method.IsStatic && !BeExtend) ? className : string.Format("{0}obj{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            ...
            if (retType == typeof(void))
            {
                if (HasSetIndex())
                {
                    ...
                    {
                        sb.AppendFormat("{0}{1}.Item = {2}arg0{3};\r\n", head, obj, pi.Name, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                    }
                    else
                    {
                ...
                {
                    sb.AppendFormat("{0}{1}.{2} = {3}arg0{4};\r\n", head, obj, pi.Name, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
                else
                {
            ...
            else
            {
                ...
                else if (HasGetIndex())
                {
                    if (methodType == 2)
                    {
                        sb.AppendFormat("{0}{1} {4}o{5} = {2}[{3}];\r\n", head, ret, obj, sbArgs.ToString(), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                    }
                    else if (methodType == 1)
                    {
                        sb.AppendFormat("{0}{1} {3}o{4} = {2}.Item;\r\n", head, ret, obj, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                    }
                    else
                    {
                        sb.AppendFormat("{0}{1} {5}o{6} = {2}.{3}({4});\r\n", head, ret, obj, method.Name, sbArgs.ToString(), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                    }
                }
                else if (method.Name == "Equals")
                {
                    ...
                    {
                        sb.AppendFormat("{0}{1} {3}o{4} = {3}obj{4}.Equals({2});\r\n", head, ret, sbArgs.ToString(), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                    }
                    else
                    {
                        sb.AppendFormat("{0}{1} {3}o{4} = {3}obj{4} != null ? {3}obj{4}.Equals({2}) : {3}arg0{4} == null;\r\n", head, ret, sbArgs.ToString(), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                    }
                }
                else if (methodType == 1)
                {
                    sb.AppendFormat("{0}{1} {4}o{5} = {2}.{3};\r\n", head, ret, obj, pi.Name, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
                else
                {
                    sb.AppendFormat("{0}{1} {5}o{6} = {2}.{3}({4});\r\n", head, ret, obj, method.Name, sbArgs.ToString(), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                ...
                GenPushStr(retType, string.Format("{0}o{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName), head, isbuffer);
            }
            ...
            for (int i = 0; i < refList.Count; i++)
            {
                ...
                {
                    sb.AppendFormat("{0}if ({2}o{3}) ToLua.Push(L{3}, {1}); else LuaDLL.lua_pushnil(L{3});\r\n", head, refList[i], CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
                else
                {
                    GenPushStr(refTypes[i], refList[i], head);
            ...
            {
                sb.Append(head + string.Format("ToLua.SetBack(L{1}, 1, {0}obj{1});\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
            }
    ...
    {
        sb.AppendFormat("public class {1}{0}Wrap{2}\r\n", wrapClassName, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName);
        sb.AppendLineEx("{");
    }
    ...
    {
        sb.AppendLineEx("}\r\n");
        SaveFile(dir + string.Format("{1}{0}Wrap{2}.cs", wrapClassName, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName));
    }
    ...
    static void GenRegisterFuncItems()
    {
        ...
        for (int i = 0; i < methods.Count; i++)
        {
            ...
            if (!nameCounter.TryGetValue(name, out count))
            {
                ...
                {
                    sb.AppendFormat("\t\tL{1}.RegFunction(\"{0}\", {2}get_Item{3});\r\n", ".geti", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
                }
                else if (name == "set_Item" && IsThisArray(m.Method, 2))
                {
                    sb.AppendFormat("\t\tL{1}.RegFunction(\"{0}\", {2}set_Item{3});\r\n", ".seti", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
                }

                if (!name.StartsWith("op_"))
                {
                    sb.AppendFormat("\t\tL{2}.RegFunction(\"{0}\", {3}{1}{4});\r\n", name, name == "Register" ? "_Register" : name, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
                }
        ...
        {
            sb.AppendFormat("\t\tL{1}.RegFunction(\"New\", {2}_Create{0}{3});\r\n", wrapClassName, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if (getItems.Count > 0 || setItems.Count > 0)
        {
            sb.AppendFormat("\t\tL{0}.RegVar(\"this\", {1}_this{2}, null);", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }
    }
    ...
    static void GenRegisterOpItems()
    {
        if ((op & MetaOp.Add) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__add\", {1}op_Addition{2});", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if ((op & MetaOp.Sub) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__sub\", {1}op_Subtraction{2});", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if ((op & MetaOp.Mul) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__mul\", {1}op_Multiply{2});", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if ((op & MetaOp.Div) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__div\", {1}op_Division{2});", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if ((op & MetaOp.Eq) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__eq\", {1}op_Equality{2});", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if ((op & MetaOp.Neg) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__unm\", {1}op_UnaryNegation{2});", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }

        if ((op & MetaOp.ToStr) != 0)
        {
            sb.AppendFormat("\t\tL{0}.RegFunction(\"__tostring\", ToLua.op_ToString);", CustomNameSetting.variableSuffixName);
        }
    }
    ...
    static void GenRegisterVariables()
    {
        ...
        for (int i = 0; i < fields.Length; i++)
        {
            if (fields[i].IsLiteral || fields[i].IsPrivate || fields[i].IsInitOnly)
            {
                if (fields[i].IsLiteral && fields[i].FieldType.IsPrimitive && !fields[i].FieldType.IsEnum)
                {
                    ...
                    sb.AppendFormat("\t\tL{2}.RegConstant(\"{0}\", {1});\r\n", fields[i].Name, d, CustomNameSetting.variableSuffixName);
                }
                else
                {
                    sb.AppendFormat("\t\tL{1}.RegVar(\"{0}\", {2}get_{0}{3}, null);\r\n", fields[i].Name, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
                }
            }
            else
            {
                sb.AppendFormat("\t\tL{1}.RegVar(\"{0}\", {2}get_{0}{3}, {2}set_{0}{3});\r\n", fields[i].Name, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            }
        }
        ...
        for (int i = 0; i < props.Length; i++)
        {
            if (props[i].CanRead && props[i].CanWrite && props[i].GetSetMethod(true).IsPublic)
            {
                ...
                sb.AppendFormat("\t\tL{3}.RegVar(\"{0}\", {4}{1}_{0}{5}, {4}{2}_{0}{5});\r\n", props[i].Name, get, set, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            }
            else if (props[i].CanRead)
            {
                _MethodBase md = methods.Find((p) => { return p.Name == "get_" + props[i].Name; });
                sb.AppendFormat("\t\tL{2}.RegVar(\"{0}\", {3}{1}_{0}{4}, null);\r\n", props[i].Name, md == null ? "get" : "_get", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            }
            else if (props[i].CanWrite)
            {
                _MethodBase md = methods.Find((p) => { return p.Name == "set_" + props[i].Name; });
                sb.AppendFormat("\t\tL{2}.RegVar(\"{0}\", null, {3}{1}_{0}{4});\r\n", props[i].Name, md == null ? "set" : "_set", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            }
        }

        for (int i = 0; i < events.Length; i++)
        {
            sb.AppendFormat("\t\tL{1}.RegVar(\"{0}\", {2}get_{0}{3}, {2}set_{0}{3});\r\n", events[i].Name, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }
    }
    ...
    static void GenRegisterEventTypes()
    {
        ...
        foreach (Type t in eventSet)
        {
            ...
            sb.AppendFormat("\t\tL{2}.RegFunction(\"{0}\", {3}{1}{4});\r\n", abr, funcName, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }
    ...
    {
        sb.AppendFormat("\tpublic static void {0}Register{1}(LuaState L{2})", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");

        if (isStaticClass)
        {
            sb.AppendFormat("\t\tL{1}.BeginStaticLibs(\"{0}\");\r\n", libClassName, CustomNameSetting.variableSuffixName);
        }
        else if (!type.IsGenericType)
        {
            if (baseType == null)
            {
                sb.AppendFormat("\t\tL{1}.BeginClass(typeof({0}), null);\r\n", className, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("\t\tL{2}.BeginClass(typeof({0}), typeof({1}));\r\n", className, GetBaseTypeStr(baseType), CustomNameSetting.variableSuffixName);
            }
        }
        else
        {
            if (baseType == null)
            {
                sb.AppendFormat("\t\tL{2}.BeginClass(typeof({0}), null, \"{1}\");\r\n", className, libClassName, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("\t\tL{3}.BeginClass(typeof({0}), typeof({1}), \"{2}\");\r\n", className, GetBaseTypeStr(baseType), libClassName, CustomNameSetting.variableSuffixName);
            }
        }
        ...
        if (!isStaticClass)
        {
            ...
            {
                sb.AppendFormat("\t\tL{0}.RegVar(\"out\", {1}get_out{2}, null);", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            }

            sb.AppendFormat("\t\tL{0}.EndClass();\r\n", CustomNameSetting.variableSuffixName);
        }
        else
        {
            sb.AppendFormat("\t\tL{0}.EndStaticLibs();\r\n", CustomNameSetting.variableSuffixName);
        }
    ...
    static String StrReplace(string strfun)
    {
       strfun = strfun.Replace("count", string.Format("{0}count{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
       strfun = strfun.Replace("arg0", string.Format("{0}arg0{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
       strfun = strfun.Replace("arg1", string.Format("{0}arg1{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
       strfun = strfun.Replace("arg2", string.Format("{0}arg2{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
       strfun = strfun.Replace("L0", string.Format("L0{0}", CustomNameSetting.variableSuffixName));
       strfun = strfun.Replace("(L)", string.Format("(L{0})", CustomNameSetting.variableSuffixName));
       strfun = strfun.Replace("(L,", string.Format("(L{0},", CustomNameSetting.variableSuffixName));
        strfun = strfun.Replace("= L;", string.Format("= L{0};", CustomNameSetting.variableSuffixName));
        return strfun;
    }
    ...
    static void GenFunction(_MethodBase m)
    {
        ...
        sb.AppendFormat("\tstatic int {1}{0}{2}(IntPtr L{3})\r\n", name == "Register" ? "_Register" : name, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        ...
        if (!haveParams)
        {
            ...
            sb.AppendFormat("\t\t\tToLua.CheckArgsCount(L{1}, {0});\r\n", count, CustomNameSetting.variableSuffixName);
        }
        else
        {
            sb.AppendFormat("\t\t\tint {0}count{1} = LuaDLL.lua_gettop(L{1});", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
    ...
    {
        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {1}_Create{0}{2}(IntPtr L{3})\r\n", wrapClassName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\t{0} {1}obj{2} = new {0}();\r\n", className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        GenPushStr(type, string.Format("{0}obj{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName), "\t\t");
    ...
    static string GetCountStr(int count)
    {
        if (count != 0)
        {
            return string.Format("{1}count{2} - {0}", count, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }

        return string.Format("{0}count{1}",CustomNameSetting.variablePrefixName,CustomNameSetting.variableSuffixName);
    }
    ...
    static void GenOutFunction()
    {
        ...
        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {0}get_out{1}(IntPtr L{2})", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\tToLua.PushOut<{0}>(L{1}, new LuaOut<{0}>());\r\n", className, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\treturn 1;");
        sb.AppendLineEx("\t}");
    }
    ...
    static void GenConstructFunction()
    {
        ...
        if (HasOptionalParam(md.GetParameters()))
        {
            ...
            if (paramInfos.Length > 1)
            {
                ...
                sb.AppendFormat("\t\t\tif (TypeChecker.CheckTypes<{0}>(L{4}, 1) && TypeChecker.CheckParamsType<{1}>(L{4}, {2}, {3}))\r\n", strParams, str, paramInfos.Length, GetCountStr(paramInfos.Length - 1), CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("\t\t\tif (TypeChecker.CheckParamsType<{0}>(L{3}, {1}, {2}))\r\n", str, paramInfos.Length, GetCountStr(paramInfos.Length - 1), CustomNameSetting.variableSuffixName);
            }
        }
        else
        {
            ...
            {
                sb.AppendFormat("\t\t\tif ({1}count{2} == {0})\r\n", paramInfos.Length, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
            else
            {
                string strParams = md.GenParamTypes(checkTypeMap[0]);
                sb.AppendFormat("\t\t\tif ({3}count{4} == {0} && TypeChecker.CheckTypes<{1}>(L{4}, {2}))\r\n", paramInfos.Length, strParams, checkTypeMap[0], CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
        }
        ...
        for (int i = 1; i < ctorList.Count; i++)
        {
            ...
            if (!HasOptionalParam(md.GetParameters()))
            {
                ...
                {
                    sb.AppendFormat("\t\t\telse if ({3}count{4} == {0} && TypeChecker.CheckTypes<{1}>(L{4}, {2}))\r\n", paramInfos.Length, strParams, checkTypeMap[i], CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
                else
                {
                    sb.AppendFormat("\t\t\telse if ({1}count{2} == {0})\r\n", paramInfos.Length, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
            }
            else
            {
                ...
                {
                    string strParams = md.GenParamTypes(1);
                    sb.AppendFormat("\t\t\telse if (TypeChecker.CheckTypes<{0}>(L{4}, 1) && TypeChecker.CheckParamsType<{1}>(L{4}, {2}, {3}))\r\n", strParams, str, paramInfos.Length, GetCountStr(paramInfos.Length - 1), CustomNameSetting.variableSuffixName);
                }
                else
                {
                    sb.AppendFormat("\t\t\telse if (TypeChecker.CheckParamsType<{0}>(L{3}, {1}, {2}))\r\n", str, paramInfos.Length, GetCountStr(paramInfos.Length - 1), CustomNameSetting.variableSuffixName);
                }
            }
        ...
        {
            sb.AppendFormat("\t\t\telse if ({0}count{1} == 0)", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t\t\t{");
            sb.AppendFormat("\t\t\t\t{0} {1}obj{2} = new {0}();\r\n", className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            GenPushStr(type, string.Format("{0}obj{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName), "\t\t\t\t");
            sb.AppendLineEx("\t\t\t\treturn 1;");
        ...
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\treturn LuaDLL.luaL_throw(L{1}, \"invalid arguments to ctor method: {0}.New\");\r\n", className, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}");
    ...
    static void GenItemPropertyFunction()
    {
        ...
        {
            sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
            sb.AppendFormat("\tstatic int _get_this(IntPtr L{0})", CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t{");
            BeginTry();
            ...
            else
            {
                ...
                sb.AppendFormat("\t\t\tint count = LuaDLL.lua_gettop(L{0});", CustomNameSetting.variableSuffixName);
                sb.AppendLineEx();
                ...
                sb.AppendLineEx("\t\t\telse");
                sb.AppendLineEx("\t\t\t{");
                sb.AppendFormat("\t\t\t\treturn LuaDLL.luaL_throw(L{1}, \"invalid arguments to operator method: {0}.this\");\r\n", className, CustomNameSetting.variableSuffixName);
                sb.AppendLineEx("\t\t\t}");
            }
        ...
        {
            sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
            sb.AppendFormat("\tstatic int _set_this(IntPtr L{0})", CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t{");
            BeginTry();
            ...
            else
            {
                ...
                sb.AppendLineEx("\t\t\telse");
                sb.AppendLineEx("\t\t\t{");
                sb.AppendFormat("\t\t\t\treturn LuaDLL.luaL_throw(L{0}, \"invalid arguments to operator method: {0}.this\");\r\n", className, CustomNameSetting.variableSuffixName);
                sb.AppendLineEx("\t\t\t}");
            }
        ...
        {
            sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
            sb.AppendFormat("\tstatic int {1}_this{2}(IntPtr L{0})", CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            sb.AppendLineEx("\t{");
            BeginTry();
            sb.AppendFormat("\t\t\tLuaDLL.lua_pushvalue(L{0}, 1);", CustomNameSetting.variableSuffixName);
            sb.AppendFormat("\t\t\tLuaDLL.tolua_bindthis(L{2}, {0}, {1});\r\n", (flag & 1) == 1 ? "_get_this" : "null", (flag & 2) == 2 ? "_set_this" : "null", CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t\t\treturn 1;");
            EndTry();
            sb.AppendLineEx("\t}");
        }
    ...
    static void CheckObject(string head, Type type, string className, int pos)
    {
        ...
        {
            sb.AppendFormat("{0}object {2}obj{3} = ToLua.CheckObject(L{3}, {1});\r\n", head, pos, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        else if (type == typeof(Type))
        {
            sb.AppendFormat("{0}{1} {3}obj{4} = ToLua.CheckMonoType(L{4}, {2});\r\n", head, className, pos, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        else if (IsIEnumerator(type))
        {
            sb.AppendFormat("{0}{1} {3}obj{4} = ToLua.CheckIter(L{4}, {2});\r\n", head, className, pos, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        else
        {
            if (IsSealedType(type))
            {
                sb.AppendFormat("{0}{1}  {3}obj{4}  = ({1})ToLua.CheckObject(L{4}, {2}, typeof({1}));\r\n", head, className, pos, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("{0}{1}  {3}obj{4}  = ({1})ToLua.CheckObject<{1}>(L{4}, {2});\r\n", head, className, pos, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
        }
    }
    ...
    static void ToObject(string head, Type type, string className, int pos)
    {
        if (type == typeof(object))
        {
            sb.AppendFormat("{0}object {2}obj{3} = ToLua.ToObject(L{3}, {1});\r\n", head, pos, CustomNameSetting.variablePrefixName,  CustomNameSetting.variableSuffixName);
        }
        else
        {
            sb.AppendFormat("{0}{1} {3}obj{4} = ({1})ToLua.ToObject(L{4}, {2});\r\n", head, className, pos, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
    }
    ...
    {
        sb.AppendLineEx("\t\t}");
        sb.AppendFormat("\t\tcatch (Exception e{0})", CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t{");
        sb.AppendFormat("\t\t\treturn LuaDLL.toluaL_exception(L{0}, e{0});", CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t}");
    }
    ...
    static void ProcessArg(Type varType, string head, string arg, int stackPos, bool beCheckTypes = false, bool beParams = false, bool beOutArg = false)
    {
        ...
        else if (varType == typeof(bool))
        {
            ...
            sb.AppendFormat("{0}bool {1} = LuaDLL.{2}(L{4}, {3});\r\n", head, arg, chkstr, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(string))
        {
            sb.AppendFormat("{0}string {1} = ToLua.{2}String(L{4}, {3});\r\n", head, arg, checkStr, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(IntPtr))
        {
            sb.AppendFormat("{0}{1} {2} = ToLua.CheckIntPtr(L{4}, {3});\r\n", head, str, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(long))
        {
            string chkstr = beCheckTypes ? "tolua_toint64" : "tolua_checkint64";
            sb.AppendFormat("{0}{1} {2} = LuaDLL.{3}(L{5}, {4});\r\n", head, str, arg, chkstr, stackPos, CustomNameSetting.variableSuffixName);
        }
        ...
        else if (varType.IsPrimitive || IsNumberEnum(varType))
        {
            ...
            sb.AppendFormat("{0}{1} {2} = ({1})LuaDLL.{3}(L{5}, {4});\r\n", head, str, arg, chkstr, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(LuaFunction))
        {
            sb.AppendFormat("{0}LuaFunction {1} = ToLua.{2}LuaFunction(L{4}, {3});\r\n", head, arg, checkStr, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType.IsSubclassOf(typeof(System.MulticastDelegate)))
        {
            if (beCheckTypes)
            {
                sb.AppendFormat("{0}{1} {2} = ({1})ToLua.ToObject(L{4}, {3});\r\n", head, str, arg, stackPos, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("{0}{1} {2} = ({1})ToLua.CheckDelegate<{1}>(L{4}, {3});\r\n", head, str, arg, stackPos, CustomNameSetting.variableSuffixName);
            }
        }
        else if (varType == typeof(LuaTable))
        {
            sb.AppendFormat("{0}LuaTable {1} = ToLua.{2}LuaTable(L{4}, {3});\r\n", head, arg, checkStr, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Vector2))
        {
            sb.AppendFormat("{0}UnityEngine.Vector2 {1} = ToLua.ToVector2(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Vector3))
        {
            sb.AppendFormat("{0}UnityEngine.Vector3 {1} = ToLua.ToVector3(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Vector4))
        {
            sb.AppendFormat("{0}UnityEngine.Vector4 {1} = ToLua.ToVector4(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Quaternion))
        {
            sb.AppendFormat("{0}UnityEngine.Quaternion {1} = ToLua.ToQuaternion(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Color))
        {
            sb.AppendFormat("{0}UnityEngine.Color {1} = ToLua.ToColor(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Ray))
        {
            sb.AppendFormat("{0}UnityEngine.Ray {1} = ToLua.ToRay(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Bounds))
        {
            sb.AppendFormat("{0}UnityEngine.Bounds {1} = ToLua.ToBounds(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(LayerMask))
        {
            sb.AppendFormat("{0}UnityEngine.LayerMask {1} = ToLua.ToLayerMask(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(object))
        {
            sb.AppendFormat("{0}object {1} = ToLua.ToVarObject(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(LuaByteBuffer))
        {
            sb.AppendFormat("{0}LuaByteBuffer {1} = new LuaByteBuffer(ToLua.CheckByteBuffer(L{3}, {2}));\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
        }
        else if (varType == typeof(Type))
        {
            if (beCheckTypes)
            {
                sb.AppendFormat("{0}System.Type {1} = (System.Type)ToLua.ToObject(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("{0}System.Type {1} = ToLua.CheckMonoType(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
            }
        }
        else if (IsIEnumerator(varType))
        {
            if (beCheckTypes)
            {
                sb.AppendFormat("{0}System.Collections.IEnumerator {1} = (System.Collections.IEnumerator)ToLua.ToObject(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("{0}System.Collections.IEnumerator {1} = ToLua.CheckIter(L{3}, {2});\r\n", head, arg, stackPos, CustomNameSetting.variableSuffixName);
            }
        }
        else if (varType.IsArray && varType.GetArrayRank() == 1)
        {
            ...
            if (flag)
            {
                if (beParams)
                {
                    if (!isObject)
                    {
                        sb.AppendFormat("{0}{1}[] {2} = ToLua.{3}<{1}>(L{6}, {4}, {5});\r\n", head, atstr, arg, fname, stackPos, GetCountStr(stackPos - 1), CustomNameSetting.variableSuffixName);
                    }
                    else
                    {
                        sb.AppendFormat("{0}object[] {1} = ToLua.{2}(L{5}, {3}, {4});\r\n", head, arg, fname, stackPos, GetCountStr(stackPos - 1), CustomNameSetting.variableSuffixName);
                    }
                }
                else
                {
                    sb.AppendFormat("{0}{1}[] {2} = ToLua.{3}<{1}>(L{5}, {4});\r\n", head, atstr, arg, fname, stackPos, CustomNameSetting.variableSuffixName);
                }
            }
            else
            {
                if (beParams)
                {
                    sb.AppendFormat("{0}{1}[] {2} = ToLua.{3}(L{6}, {4}, {5});\r\n", head, atstr, arg, fname, stackPos, GetCountStr(stackPos - 1), CustomNameSetting.variableSuffixName);
                }
                else
                {
                    sb.AppendFormat("{0}{1}[] {2} = ToLua.{3}(L{5}, {4});\r\n", head, atstr, arg, fname, stackPos, CustomNameSetting.variableSuffixName);
                }
            }
        }
        else if (varType.IsGenericType && varType.GetGenericTypeDefinition() == typeof(Nullable<>))
        {
            ...
            if (beCheckTypes)
            {
                sb.AppendFormat("{0}{1} {2} = ToLua.ToNullable<{3}>(L{5}, {4});\r\n", head, str, arg, GetTypeStr(t), stackPos, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("{0}{1} {2} = ToLua.CheckNullable<{3}>(L{5}, {4});\r\n", head, str, arg, GetTypeStr(t), stackPos, CustomNameSetting.variableSuffixName);
            }
        }
        else if (varType.IsValueType && !varType.IsEnum)
        {
            string func = beCheckTypes ? "To" : "Check";
            sb.AppendFormat("{0}{1} {2} = StackTraits<{1}>.{3}(L{5}, {4});\r\n", head, str, arg, func, stackPos, CustomNameSetting.variableSuffixName);
        }
        else //从object派生但不是object
        {
            ...
            else
            {
                ...
                {
                    sb.AppendFormat("{0}{1} {2} = ({1})ToLua.CheckObject(L{4}, {3}, typeof({1}));\r\n", head, str, arg, stackPos, CustomNameSetting.variableSuffixName);
                }
                else
                {
                    sb.AppendFormat("{0}{1} {2} = ({1})ToLua.CheckObject<{1}>(L{4}, {3});\r\n", head, str, arg, stackPos, CustomNameSetting.variableSuffixName);
                }
            }
        }
    }
    ...
    static void GenPushStr(Type t, string arg, string head, bool isByteBuffer = false)
    {
        if (t == typeof(int))
        {
            sb.AppendFormat("{0}LuaDLL.lua_pushinteger(L{2},{1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else if (t == typeof(bool))
        {
            sb.AppendFormat("{0}LuaDLL.lua_pushboolean(L{2}, {1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else if (t == typeof(string))
        {
            sb.AppendFormat("{0}LuaDLL.lua_pushstring(L{2}, {1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else if (t == typeof(IntPtr))
        {
            sb.AppendFormat("{0}LuaDLL.lua_pushlightuserdata(L{2}, {1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else if (t == typeof(long))
        {
            sb.AppendFormat("{0}LuaDLL.tolua_pushint64(L{2}, {1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else if (t == typeof(ulong))
        {
            sb.AppendFormat("{0}LuaDLL.tolua_pushuint64(L{2}, {1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else if ((t.IsPrimitive))
        {
            sb.AppendFormat("{0}LuaDLL.lua_pushnumber(L{2}, {1});\r\n", head, arg, CustomNameSetting.variableSuffixName);
        }
        else
        {
            if (isByteBuffer && t == typeof(byte[]))
            {
                sb.AppendFormat("{0}LuaDLL.tolua_pushlstring(L{2}, {1}, {1}.Length);\r\n", head, arg, CustomNameSetting.variableSuffixName);
            }
            else
            {
                string str = GetPushFunction(t);
                sb.AppendFormat("{0}ToLua.{1}(L{3}, {2});\r\n", head, str, arg, CustomNameSetting.variableSuffixName);
            }
        }
    }
    ...
    static void GenOverrideFuncBody(_MethodBase md, bool beIf, int checkTypeOffset)
    {
        ...
        if (HasOptionalParam(md.GetParameters()))
        {
            ...
            if (paramInfos.Length + offset > 1)
            {
                ...
                sb.AppendFormat("\t\t\t{0}(TypeChecker.CheckTypes<{1}>(L{5}, 1) && TypeChecker.CheckParamsType<{2}>(L{5}, {3}, {4}))\r\n", strIf, strParams, str, paramInfos.Length + offset, GetCountStr(paramInfos.Length + offset - 1), CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("\t\t\t{0}(TypeChecker.CheckParamsType<{1}>(L{4}, {2}, {3}))\r\n", strIf, str, paramInfos.Length + offset, GetCountStr(paramInfos.Length + offset - 1), CustomNameSetting.variableSuffixName);
            }
        }
        else
        {
            ...
            if (paramInfos.Length + offset > checkTypeOffset)
            {
                ...
                sb.AppendFormat("\t\t\t{0}({4}count{5} == {1} && TypeChecker.CheckTypes<{2}>(L{5}, {3}))\r\n", strIf, paramInfos.Length + offset, strParams, checkTypeOffset + 1, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
            else
            {
                sb.AppendFormat("\t\t\t{0}({2}count{3} == {1})\r\n", strIf, paramInfos.Length + offset, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
        }
    ...
    static _MethodBase GenOverrideFunc(string name)
    {
        ...
        sb.AppendLineEx("\t\t\telse");
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\treturn LuaDLL.luaL_throw(L{2}, \"invalid arguments to method: {0}.{1}\");\r\n", className, name, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}");
    ...
    {
        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {2}{0}_{1}{3}(IntPtr L{4})\r\n", beOverride ? "_get" : "get", varName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        ...
        {
            sb.AppendFormat("\t\tobject {0}o{1} = null;\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            BeginTry();
            sb.AppendFormat("\t\t\t{0}o{1} = ToLua.ToObject(L{1}, 1);", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            sb.AppendFormat("\t\t\t{0} {1}obj{2} = ({0}){1}o{2};\r\n", className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            sb.AppendFormat("\t\t\t{0} {2}ret{3} = {2}obj{3}.{1};\r\n", GetTypeStr(varType), varName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            GenPushStr(varType, string.Format("{0}ret{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName), "\t\t\t", isByteBuffer);
            sb.AppendLineEx("\t\t\treturn 1;");
            ...
            sb.AppendFormat("\t\t\treturn LuaDLL.toluaL_exception(L{2}, {1}e{2}, {1}o{2}, \"attempt to index {0} on a nil value\");\r\n", varName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t\t}");
        }
    ...
    {
        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {1}get_{0}{2}(IntPtr L{3})\r\n", varName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\tToLua.Push(L{1}, new EventObject(typeof({0})));\r\n", GetTypeStr(varType), CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\treturn 1;");
        sb.AppendLineEx("\t}");
    }
    ...
    {
        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {2}{0}_{1}{3}(IntPtr L{4})\r\n", beOverride ? "_set" : "set", varName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        ...
        if (!isStatic)
        {
            ...
            sb.AppendFormat("\t\t\t{0}o{1} = ToLua.ToObject(L{1}, 1);", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            sb.AppendFormat("\t\t\t{0} {1}obj{2} = ({0}){1}o{2};\r\n", className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            ProcessArg(varType, "\t\t\t", string.Format("{0}arg0{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName), 2);
            sb.AppendFormat("\t\t\t{1}obj{2}.{0} = {1}arg0{2};\r\n", varName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);

            if (type.IsValueType)
            {
                sb.AppendFormat("\t\t\tToLua.SetBack(L{1}, 1, {0}obj{1});", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            }
            sb.AppendLineEx("\t\t\treturn 0;");
            sb.AppendLineEx("\t\t}");
            sb.AppendFormat("\t\tcatch(Exception {0}e{1})", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        ...
        else
        {
            BeginTry();
            ProcessArg(varType, "\t\t\t", string.Format("{0}arg0{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName), 2);
            sb.AppendFormat("\t\t\t{0}.{1} = {2}arg0{3};\r\n", className, varName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t\t\treturn 0;");
            EndTry();
        }
    ...
    {
        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {1}set_{0}{2}(IntPtr L{3})\r\n", varName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        BeginTry();

        if (!isStatic)
        {
            sb.AppendFormat("\t\t\t{0} {1}obj{2} = ({0})ToLua.CheckObject(L{2}, 1, typeof({0}));\r\n", className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }

        string strVarType = GetTypeStr(varType);
        string objStr = isStatic ? className : string.Format("{0}obj{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);

        sb.AppendFormat("\t\t\tEventObject {0}arg0{1} = null;\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\tif (LuaDLL.lua_isuserdata(L{0}, 2) != 0)",  CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\t{0}arg0{1} = (EventObject)ToLua.ToObject(L{1}, 2);", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}");
        sb.AppendLineEx("\t\t\telse");
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\treturn LuaDLL.luaL_throw(L{2}, \"The event '{0}.{1}' can only appear on the left hand side of += or -= when used outside of the type '{0}'\");\r\n", className, varName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}\r\n");

        sb.AppendFormat("\t\t\tif ({0}arg0{1}.op == EventOp.Add)", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\t{0} {1}ev{2} = ({0}){1}arg0{2}.func;\r\n", strVarType, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\t\t{0}.{1} += {2}ev{3};\r\n", objStr, varName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}");
        sb.AppendFormat("\t\t\telse if ({0}arg0{1}.op == EventOp.Sub)", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\t{0} {1}ev{2} = ({0}){1}arg0{2}.func;\r\n", strVarType, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\t\t{0}.{1} -= {2}ev{3};\r\n", objStr, varName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}\r\n");
    ...
    static void GenDelegateBody(StringBuilder sb, Type t, string head, bool hasSelf = false)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (!IsParams(pi[i]))
            {
                if (pi[i].ParameterType == typeof(byte[]) && IsByteBuffer(t))
                {
                    sb.AppendFormat("{2}\tfunc.PushByteBuffer({3}param{4}{1});\r\n", push, i, head, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
                else if (pi[i].Attributes != ParameterAttributes.Out)
                {
                    sb.AppendFormat("{2}\tfunc.{0}({3}param{4}{1});\r\n", push, i, head, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                }
            }
            else
            {
                sb.AppendLineEx();
                sb.AppendFormat("{0}\tfor (int i = 0; i < {2}param{3}{1}.Length; i++)\r\n", head, i, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                sb.AppendLineEx(head + "\t{");
                sb.AppendFormat("{2}\t\tfunc.{0}({3}param{4}{1}[i]);\r\n", push, i, head, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
                sb.AppendLineEx(head + "\t}\r\n");
            }
        }
        ...
        if (mi.ReturnType == typeof(void))
        {
            for (int i = 0; i < pi.Length; i++)
            {
                if ((pi[i].Attributes & ParameterAttributes.Out) != ParameterAttributes.None)
                {
                    GenLuaFunctionRetValue(sb, pi[i].ParameterType.GetElementType(), head + "\t", string.Format("{0}param{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName) + i, true);
                }
            }
    ...
    static void CallOpFunction(string name, int count, string ret)
    {
        ...
        if (name == "op_Addition")
        {
            sb.AppendFormat("{0}{1} {2}o{3} = {2}arg0{3} + {2}arg1{3};\r\n", head, ret, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        else if (name == "op_Subtraction")
        {
            sb.AppendFormat("{0}{1} {2}o{3} = {2}arg0{3} - {2}arg1{3};\r\n", head, ret, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        ...
        {
            sb.AppendFormat("{0}{1} {2}o{3} = -{2}arg0{3};\r\n", head, ret, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
    }
    ...
    static void GenEnum()
    {
        ...
        sb.AppendFormat("\tpublic static void {0}Register{1}(LuaState L{2})", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\tL{1}.BeginEnum(typeof({0}));\r\n", className, CustomNameSetting.variableSuffixName);

        for (int i = 0; i < fields.Length; i++)
        {
            sb.AppendFormat("\t\tL{3}.RegVar(\"{0}\", {1}get_{0}{2}, null);\r\n", fields[i].Name, CustomNameSetting.enumPrefixName, CustomNameSetting.enumSuffixName, CustomNameSetting.variableSuffixName);
        }

        sb.AppendFormat("\t\tL{2}.RegFunction(\"IntToEnum\", {0}IntToEnum{1});\r\n", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\tL{0}.EndEnum();\r\n", CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t//dddddd");
        sb.AppendFormat("\t\tTypeTraits<{0}>.Check = {1}CheckType{2};\r\n", className, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        sb.AppendFormat("\t\tStackTraits<{0}>.Push = {1}Push{2};\r\n", className, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        sb.AppendLineEx("\t}");
        sb.AppendLineEx();

        sb.AppendFormat("\tstatic void {1}Push{2}(IntPtr L{3}, {0} arg{3})\r\n", className, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\tToLua.Push(L{0}, arg{0});", CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t}");
        sb.AppendLineEx();

        sb.AppendFormat("\tstatic bool {0}CheckType{1}(IntPtr L{2}, int pos{2})", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\treturn TypeChecker.CheckEnumType(typeof({0}), L{1}, pos{1});\r\n", className, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t}");
        ...
        for (int i = 0; i < fields.Length; i++)
        {
            ...
            sb.AppendFormat("\tstatic int {1}get_{0}{2}(IntPtr L{3})\r\n", fields[i].Name, CustomNameSetting.enumPrefixName, CustomNameSetting.enumSuffixName, CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t{");
            sb.AppendFormat("\t\tToLua.Push(L{2}, {0}.{1});\r\n", className, fields[i].Name, CustomNameSetting.variableSuffixName);
            sb.AppendLineEx("\t\treturn 1;");
            sb.AppendLineEx("\t}");
        }

        sb.AppendLineEx("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendFormat("\tstatic int {0}IntToEnum{1}(IntPtr L{2})", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\tint {0}arg0{1} = (int)LuaDLL.lua_tonumber(L{1}, 1);", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t{0} {1}o{2} = ({0}){1}arg0{2};\r\n", className, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\tToLua.Push(L{1}, {0}o{1});", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\treturn 1;");
        sb.AppendLineEx("\t}");
    }
...
";
    ...
    static string GetDelegateParams(MethodInfo mi)
    {
        ...
        for (int i = 0; i < infos.Length; i++)
        {
            string s2 = GetTypeStr(infos[i].ParameterType) + string.Format(" {0}param{1}", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName) + i;
    ...
    static string GetDefaultDelegateBody(MethodInfo md)
    {
        ...
        for (int i = 0; i < pis.Length; i++)
        {
            ...
            {
                str += string.Format("\t\t\t\t{2}param{0} = {1};\r\n", i, GetReturnValue(pis[i].ParameterType.GetElementType()), CustomNameSetting.publicPrefixName);
                flag = true;
            }
        }
    ...
    public static void GenDelegates(DelegateType[] list)
    {
        ...
        sb.Append(string.Format("public class DelegateFactory\r\n"));
        sb.Append("{\r\n");
        sb.Append(string.Format("\tpublic delegate Delegate DelegateCreate(LuaFunction func{0}, LuaTable self{0}, bool flag{0});\r\n", CustomNameSetting.publicSuffixName));
        sb.Append(string.Format("\tpublic static Dictionary<Type, DelegateCreate> _dict = new Dictionary<Type, DelegateCreate>();\r\n"));
        sb.Append(string.Format("\tstatic DelegateFactory {0}factory{1} = new DelegateFactory();\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
        sb.AppendLineEx();
        sb.Append("\tpublic static void Init()\r\n");
        sb.Append("\t{\r\n");
        sb.Append(string.Format("\t\t{0}Register{1}();\r\n", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName));
        sb.AppendLineEx("\t}\r\n");

        sb.Append(string.Format("\tpublic static void {0}Register{1}()\r\n", CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName));
        sb.Append("\t{\r\n");
        sb.Append(string.Format("\tDictionary<Type, DelegateCreate> {0}dict{1} = _dict;\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName));
        sb.AppendFormat("\t\t{0}dict{1}.Clear();\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);

        for (int i = 0; i < list.Length; i++)
        {
            string type = list[i].strType;
            string name = list[i].name;
            sb.AppendFormat("\t\t{2}dict{3}.Add(typeof({0}), {2}factory{3}.{4}{1}{5});\r\n", type, name, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
        }
        ...
        for (int i = 0; i < list.Length; i++)
        {
            string type = list[i].strType;
            string name = list[i].name;
            sb.AppendFormat("\t\tDelegateTraits<{0}>.Init({4}factory{5}.{2}{1}{3});\r\n", type, name, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        ...
        for (int i = 0; i < list.Length; i++)
        {
            string type = list[i].strType;
            string name = list[i].name;
            sb.AppendFormat("\t\tTypeTraits<{0}>.Init({4}factory{5}.{2}Check_{1}{3});\r\n", type, name, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        ...
        for (int i = 0; i < list.Length; i++)
        {
            string type = list[i].strType;
            string name = list[i].name;
            sb.AppendFormat("\t\tStackTraits<{0}>.Push = {4}factory{5}.{2}Push_{1}{3};\r\n", type, name, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        }
        ...
        for (int i = 0; i < list.Length; i++)
        {
            ...
            //生成委托类
            sb.AppendFormat("\tclass {1}{0}{2}_Event : LuaDelegate\r\n", name, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName);
            sb.AppendLineEx("\t{");
            sb.AppendFormat("\t\tpublic {1}{0}{2}_Event(LuaFunction func{3}) : base(func{3}) {{ }}\r\n", name, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName, CustomNameSetting.publicSuffixName);
            sb.AppendFormat("\t\tpublic {1}{0}{2}_Event(LuaFunction func{3}, LuaTable self{3}) : base(func{3}, self{3}) {{ }}\r\n", name, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName, CustomNameSetting.publicSuffixName);
            sb.AppendLineEx();
            sb.AppendFormat("\t\tpublic {0} {2}Call{3}({1})\r\n", GetTypeStr(mi.ReturnType), args, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            GenDelegateBody(sb, t, "\t\t");
            sb.AppendLineEx();
            sb.AppendFormat("\t\tpublic {0} {2}CallWithSelf{3}({1})\r\n", GetTypeStr(mi.ReturnType), args, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
            GenDelegateBody(sb, t, "\t\t", true);
            sb.AppendLineEx("\t}\r\n");

            //生成转换函数1
            sb.AppendFormat("\tpublic {0} {2}{1}{3}(LuaFunction func{4}, LuaTable self{4}, bool flag{4})\r\n", strType, name, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.publicSuffixName);
            sb.AppendLineEx("\t{");
            sb.AppendLineEx(string.Format("\t\tif (func{0} == null)", CustomNameSetting.publicSuffixName));
            sb.AppendLineEx("\t\t{");
            sb.AppendFormat("\t\t\t{0} fn{3} = delegate({1}) {2}", strType, args, GetDefaultDelegateBody(mi), CustomNameSetting.publicSuffixName);
            sb.AppendLineEx(string.Format("\t\t\treturn fn{0};", CustomNameSetting.publicSuffixName));
            sb.AppendLineEx("\t\t}\r\n");
            sb.AppendLineEx(string.Format("\t\tif(!flag{0})", CustomNameSetting.publicSuffixName));
            sb.AppendLineEx("\t\t{");
            sb.AppendFormat("\t\t\t{3}{0}{4}_Event {2}target = new {3}{0}{4}_Event(func{1});\r\n", name, CustomNameSetting.publicSuffixName, CustomNameSetting.publicPrefixName, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName);
            sb.AppendFormat("\t\t\t{0} {3}d = {3}target.{1}Call{2};\r\n", strType, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.publicPrefixName);
            sb.AppendLineEx(string.Format("\t\t\t{0}target.method = {0}d.Method;", CustomNameSetting.publicPrefixName));
            sb.AppendFormat("\t\t\treturn {0}d;", CustomNameSetting.publicPrefixName);
            sb.AppendLineEx("\t\t}");
            sb.AppendLineEx("\t\telse");
            sb.AppendLineEx("\t\t{");
            sb.AppendFormat("\t\t\t{2}{0}{3}_Event {1}target = new {2}{0}{3}_Event(func{4}, self{4});\r\n", name, CustomNameSetting.publicPrefixName, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName, CustomNameSetting.publicSuffixName);
            sb.AppendFormat("\t\t\t{0} {3}d = {3}target.{1}CallWithSelf{2};\r\n", strType, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.publicPrefixName);
            sb.AppendFormat("\t\t\t{0}target.method = {0}d.Method;", CustomNameSetting.publicPrefixName);
            sb.AppendFormat("\t\t\treturn {0}d;", CustomNameSetting.publicPrefixName);
            sb.AppendLineEx("\t\t}");
            sb.AppendLineEx("\t}\r\n");

            sb.AppendFormat("\tbool {1}Check_{0}{2}(IntPtr L{3}, int pos{3})\r\n", name, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.publicSuffixName);
            sb.AppendLineEx("\t{");
            sb.AppendFormat("\t\treturn TypeChecker.CheckDelegateType(typeof({0}), L{1}, pos{1});\r\n", strType, CustomNameSetting.publicSuffixName);
            sb.AppendLineEx("\t}\r\n");

            sb.AppendFormat("\tvoid {2}Push_{0}{3}(IntPtr L{4}, {1} o{4})\r\n", name, strType, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName, CustomNameSetting.publicSuffixName);
            sb.AppendLineEx("\t{");
            sb.AppendFormat("\t\tToLua.Push(L{0}, o{0});", CustomNameSetting.publicSuffixName);
            sb.AppendLineEx("\t}\r\n");
        }

        sb.AppendLineEx("}\r\n");
        SaveFile(CustomSettings.saveDir + string.Format("DelegateFactory.cs"));
    ...
    public static void GenEventFunction(Type t, StringBuilder sb)
    {
        ...
        sb.AppendLineEx("\t\t{");
        sb.AppendFormat("\t\t\tint {0}count{1} = LuaDLL.lua_gettop(L{1});", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\tLuaFunction {0}func{1} = ToLua.CheckLuaFunction(L{1}, 1);", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx();
        sb.AppendFormat("\t\t\tif ({0}count{1} == 1)", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\tDelegate {1}arg1{2} = DelegateTraits<{0}>.Create({1}func{2});\r\n", GetTypeStr(t), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\t\tToLua.Push(L{1}, {0}arg1{1});", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}");
        sb.AppendLineEx("\t\t\telse");
        sb.AppendLineEx("\t\t\t{");
        sb.AppendFormat("\t\t\t\tLuaTable {0}self{1} = ToLua.CheckLuaTable(L{1}, 2);", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\t\tDelegate {1}arg1{2} = DelegateTraits<{0}>.Create({1}func{2}, {1}self{2});\r\n", GetTypeStr(t), CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\t\t\tToLua.Push(L{1}, {0}arg1{1});\r\n", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t\t\t}");
