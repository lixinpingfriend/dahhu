...
public static class ToLuaMenu
{
    ...
    static void GenLuaBinder()
    {
        ...
        sb.AppendLineEx("{");
        sb.AppendFormat("\tpublic static void Bind(LuaState L{0})", CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t{");
        sb.AppendFormat("\t\tfloat {0}t{1} = Time.realtimeSinceStartup;", CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendFormat("\t\tL{0}.BeginModule(null);", CustomNameSetting.variableSuffixName);
        ...
        Action<ToLuaNode<string>> begin = (node) =>
        {
            ...
            sb.AppendFormat("\t\tL{1}.BeginModule(\"{0}\");\r\n", node.value, CustomNameSetting.variableSuffixName);
        ...
        Action<ToLuaNode<string>> end = (node) =>
        {
            if (node.value != null)
            {
                sb.AppendFormat("\t\tL{0}.EndModule();", CustomNameSetting.variableSuffixName);
            }
        };
        ...
        {
            sb.AppendFormat("\t\tL{0}.BeginPreLoad();", CustomNameSetting.variableSuffixName);
            ...
            for (int i = 0; i < CustomSettings.dynamicList.Count; i++)
            {
                ...
                if (bt != null) sb.AppendFormat("\t\tL{2}.AddPreLoad(\"{0}\",{3}LuaOpen_{1}{4}, typeof({0}));\r\n", bt.name, bt.wrapName, CustomNameSetting.variableSuffixName,
                    CustomNameSetting.functionPrefixName,CustomNameSetting.functionSuffixName);
            }

            sb.AppendFormat("\t\tL{0}.EndPreLoad();", CustomNameSetting.variableSuffixName);
        }

        sb.AppendFormat("\t\tDebugger.Log(\"Register lua type cost time: {0}\", Time.realtimeSinceStartup - {1}t{2});","{0}",CustomNameSetting.variablePrefixName, CustomNameSetting.variableSuffixName);
        sb.AppendLineEx("\t}");
    ...
    static void GenRegisterInfo(string nameSpace, StringBuilder sb, List<DelegateType> delegateList, List<DelegateType> wrappedDelegatesCache)
    {
        for (int i = 0; i < allTypes.Count; i++)
        {
            ...
            {
                string str = string.Format("\t\t{1}{0}Wrap{2}.{3}Register{4}(L{5});\r\n", allTypes[i].wrapName, CustomNameSetting.classPrefixName, CustomNameSetting.classSuffixName,
        ...
        for (int i = 0; i < delegateList.Count; i++)
        {
            ...
            if (typeSpace == nameSpace)
            {
                ...
                sb.AppendFormat("\t\tL{2}.RegFunction(\"{0}\", {3}{1}{4});\r\n", abr, dt.name, CustomNameSetting.variableSuffixName, CustomNameSetting.functionPrefixName, CustomNameSetting.functionSuffixName);
